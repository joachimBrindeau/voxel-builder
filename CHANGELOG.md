...
### Added
- **CPT-agnostic `sameAs` identity linking** — `workflows/identity-linking.md` and `references/voxel/sameas-identity-links.md` separate exact entity equivalence from citations, define the canonical URL-only Voxel repeater and flat schema projection, support Wikipedia/Wikidata and proven official identity URLs, require explicit accepted/skipped decisions for complete coverage, and provide reversible apply plus full live-verification gates for any CPT.
...
### Fixed
- **Coverage root override parity** — made `scripts/check-wpdev-coverage.py` honor `WPDEV_SOURCE_ROOT` as well as `WPDEV_ROOT`, added a read-only index-resolution diagnostic, and extended helper regression coverage so a source-only checkout can run the full lint gate.
