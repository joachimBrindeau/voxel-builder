...
- **Product Field catalog** (`product-types/product-fields/`):
  ...
  | `currency-field` | Per-product currency override. **Has no form component in this workspace — do not enable.** See the gotcha below. |
...
- **Gotchas**:
  ...
  - **An enabled product module whose form component does not exist takes the whole product form down, not just its own input.** The create-post form renders each enabled subfield as `<component :is="field.component_key">`. With no registered component, Vue treats the name as a native element and calls `setAttribute` with the bound Vue Proxy, throwing `TypeError: can't convert Proxy to string` and aborting the render — so the wp-admin **Fields** metabox comes up *entirely blank*, which reads as a broken post or a dead plugin rather than as one bad module flag. The second consequence is quieter and worse: such a subfield's `sanitize()` can veto the payload, making `Product_Field::sanitize()` return `null`, and since `Product_Field::update()` deletes the meta on an empty value, **saving the post erases its product data**. `custom_currency` is the live example — `field-currency` was removed from the create-post assets in #441 (prices are always the primary currency, asserted by `custom-fields-components.test.mjs`), but the module stayed `enabled: true` in `voxel:product_types` on best-matcha. The two sides drift independently, so neither detects it alone. Check with `./wpdev voxel:product-form <site>`, which proves every enabled subfield resolves to a shipped component or template and **exits non-zero** when one cannot render, so it can gate a change rather than merely describe one; `./wpdev voxel:status <site>` reports the same finding inline among the index health it already prints. A site with no Voxel product types reports "nothing to check" rather than passing clean, because an empty result there and an empty result from a scan that read no component sources are otherwise indistinguishable. Never conclude a module is safe to enable from `product-types.config.php` alone, since that registry lists classes rather than renderable components.
  - **To test whether a save would preserve product data, call `sanitize()` and never `set_value()`.** The obvious way to ask "would saving this post destroy it" is to replay the stored payload through the field, but `Base_Field::set_value()` routes into `Product_Field::update()`, which **writes**: it calls `prepare_for_storage()` and persists. A sweep written that way is a mass edit of every product post wearing the costume of a probe, and it announces itself only if it happens to crash. It does crash here, which is the one piece of luck in it: `update()` hands `$variation['image']` to `File_Field::_prepare_ids_from_sanitized_input()`, which expects the **sanitized form** shape (`[{source, data}]`) rather than the **stored** shape (a bare attachment ID), so it fatals with `Cannot access offset of type string on string` at `file-field-trait.php:164` partway through the loop, leaving whatever it already reached rewritten. `sanitize()` is pure and answers the same question. Prove a sweep was genuinely read-only by capturing row count, variation count, and a distinguishing key count before and after and requiring them identical; never infer it from the absence of an error, since the write path is silent when it succeeds.
  - **Read the stored payload's shape from the code that consumes it, not from where it looks like it should live.** Variation images are stored at `$variation['image']`, top level, while prices and other settings sit under `$variation['config']`. Probing `config.image` finds nothing on every post and reports a confident "no images anywhere", which is a sound instrument aimed at nothing: it cannot distinguish "this site has no variation images" from "I looked in the wrong key". `Variations_Field::update()` and `editing_value()` are the authority for both halves of the contract, stored being a numeric attachment ID and the editing form being `[{source: 'existing', ...}]`.
  - **Only `custom` attributes in `cards` or `images` display mode reach the file-field path at all.** `Variations_Field::update()` gates choice-image handling on that pair, so a site whose attributes are all `buttons`/`dropdown` can never reach `prepare_for_storage()` for choices, and a variation with a `null` image is likewise inert because `foreach ( (array) null )` is empty. Establish this before treating a fatal seen in a probe as a live user-facing bug: on best-matcha all 147 attributes are `buttons` or `dropdown`, so real-save exposure is zero, which a real admin UI save then confirmed.
