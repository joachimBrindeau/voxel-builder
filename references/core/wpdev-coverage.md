...
| `voxel:field-schema` | `surfaced` | `workflows/cpt-lifecycle.md` Phase 2; `references/core/command-surface.md` §CPT introspection | Corruption-proof runtime patch path for Voxel field definitions (e.g. length bounds / required flags). Use after updating the blueprint/source, then read back with `voxel:fields`. |
